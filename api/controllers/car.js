...
// sprawdzenie czy dany pojazd nie jest aktualnie przedmiotem nierozwiązania innego formularza(?)
// car registration 
...
export const regCar = (req, res) => {
    ...
    jwt.verify(token, secretKey, async (err, userData) => {
        ...
        db.query(q_check, [userData.id], async (err, data) => {

            if(data.rows[0].document_id === null) {
                return res.status(409).json("Żeby zarejestrować pojazd, uzupełnij informacje o dokumencie tożsamości na swoim profilu.");
            } else if(data.rows[0].address_id === null) {

                return res.status(409).json("Żeby zarejestrować pojazd, uzupełnij informacje o adresie na swoim profilu.");
                ...
                // slice, zeby zmienna DATA dostosowac do formatu 'YYYY-MM-DD'
                    ...
                    if (isNaN(production_year) || production_year < 1886) {
                        return res.status(409).json("Niepoprawny rok produkcji.");
                    }
                    ...
                    return res.status(500).json("Wystąpił błąd podczas transakcji");
